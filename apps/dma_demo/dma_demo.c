...
int main(int argc, char *argv[]) {
    ...
    if (argc > 1) {
        if (!strcmp(argv[1], "both") || !strcmp(argv[1], "b") ||
            !strcmp(argv[1], "loop")) {
            mode = 3;
        } else if (!strcmp(argv[1], "c2h") || !strcmp(argv[1], "r") ||
                   !strcmp(argv[1], "rx")) {
            mode = 2;
        }
    }
        ...
        while (c2h_count < loop || h2c_count < loop) {
            ...
            if (mode == 3) {
                ...
                if (i < cnt * 4) {
                    if (DBG_INFO)
                        fprintf(stdout,
                                "differ at offset %d: \033[31m0x%02x != "
                                "~0x%02x\033[0m \n",
                                i, sp[i], ~dp[i]);
                }
            }
